...
sed -e 's/boy/boys/g; s/like/love/g' ./data_test.txt                    # they will do according to the 2 expression

echo "Another way to handle it"
sed -e '
s/boy/boys/g
s/like/love/g
s/cute/clever/g
...
echo "sed -f script file, test for this format."                        # put the script in a file and use -f to launch it.
sed -f script1.sed ./data_test.txt                                      # it works as expectation

sed 's/test/trial/' ./data4.txt
sed 's/test/trial/1' ./data4.txt                                        # the same effect as before.
sed 's/test/trial/2' ./data4.txt                                        # the second matched items will be replaced. The number 2 means second.
